...
- **OPEN DECISION (Griffin)**: the dashboard's world-electricity denominator dict (app.py ~L1647) is systematically ~7–15% LOW vs the Energy Institute series (2024: 26,100 vs ~30,900 TWh), so the nuclear-share chart overstates share (~10.3% vs true ~8.7%). Recommended: rebase entire 1970–2025 series from EI/OWID. Until then the share chart stays capped at 2024 (2025 EI figure = 32,202 TWh, found but not applied — would create a false cliff on the wrong basis).
- **NEXT**: Griffin decides world-series rebase; push + `fly deploy` (fly CLI needs `fly auth login`); new backlog items in `pris_id_repair_2026-07.md` §backlog (capacity-history gaps: Wolsong 3/4, Hunterston B 1990s, Bruce 2 MCR; general NULL-pris_id sweep; Shidaowan Guohe One non-PRIS generation source).

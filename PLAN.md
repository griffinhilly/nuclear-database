...
- **Embed map (Jul 16–20)**: `/embed/map` — iframe-able teaser map for the REG website (yellow tint wash, click-through disabled zoom, Esri light-gray tiles). Built + deployed Jul 16; refinement pass committed Jul 20 (CTA removed, UC markers → REG cobalt, fit-to-fleet framing) — **needs `fly deploy` to go live** (live version verified stale Jul 20).

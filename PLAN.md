...
Jul 20-21, 2026 — constants audit + vendor verification shipped (see bullets). Jul 6 sweep detail + May 28 deferred items: `PLAN_current_state_archive_2026-07.md`.
...
- **Backlog** (unranked): label the current-year share point provisional on the dashboard chart (2025 = 9.3% rides on 88.6% coverage estimate) · capacity-history gaps (Wolsong 3/4 net values, Hunterston B 1990s derate, Hinkley Point B2 1994, Bruce 2 MCR uprate) · general NULL-pris_id sweep from the id map · Shidaowan Guohe One generation needs a non-PRIS source · 1970–84 world-electricity stub (UN yearbook transcription) if the share chart should extend back · China cooling fallthrough fragility in populate_reactor_details.py · 3 reactors have no reactor_details row at all (741 reactors vs 738 verified — identify + backfill attested values only).
...
- **Design-spec completion pass (Jul 21) — PAUSED, search-quota blocked**: goal = verify the ~1,800 unsampled design_series_specs values under the zero-doubt standard. 10 batches prepared (`verification_2026-07/spec_inputs/specs_01-10.psv`, prompt template in session 2026-07-21). Session's WebSearch budget was exhausted by the vendor-field program; 2 completed batches produced MEMORY-ONLY verdicts and were quarantined (`spec_results/*.tainted-memory-only` — do NOT merge), remaining 8 agents stopped before writing. **Resume in a fresh session**: canary-check WebSearch works, re-dispatch batches 01-10 with the no-memory-confirmations rule (require BLOCKED if search unavailable), merge → migration 019 (apply sourced corrections, NULL unverifiable), extend drift guard to specs. Until then design_series_specs remains visible with the sampled-only assurance (Griffin aware).
- **Drift guard (Jul 21)**: validate_db.py check 15 — any non-NULL reactor_details vendor value not whitelisted in `final_verdicts.psv` fails validation. Tested (plant-and-revert). Noah/Dirk attestations must be appended to final_verdicts.psv (visible=yes) in the same change that writes them to the DB.
- **Next session**: resume design-spec completion pass (above) · send Noah/Dirk consult package (Griffin) · then backlog / external triggers (Noah round 2, 2026 PRIS data ~mid-2027).
